...
function changeDisc(direction) {
    ...
    const cdImage = document.getElementById('cd'); // Reuse the existing image
    const genreTitle = document.getElementById('genre-title');

    // Save current song title and playback position
    const previousTrackTitle = trackTitle.textContent;
    currentSongTime = audioPlayer.currentTime;

    // Determine new disc index
    currentDiscIndex = (direction === 'next') 
        ? (currentDiscIndex + 1) % discCollection.length 
        : (currentDiscIndex - 1 + discCollection.length) % discCollection.length;

    const newDisc = discCollection[currentDiscIndex];

    // Extract and update genre title
    genreTitle.textContent = newDisc.img.split('/')[0];
    genreTitle.style.opacity = '0'; // Start fading out

    // Create new image for transition effect
    const newCdImage = document.createElement('img');
    newCdImage.src = newDisc.img;
    newCdImage.classList.add('cd');
    newCdImage.style.position = 'absolute';
    newCdImage.style.opacity = '0';
    newCdImage.style.transform = `translateX(${direction === 'next' ? '100%' : '-100%'}) scale(1.1)`;
    newCdImage.style.transition = 'transform 0.6s ease-out, opacity 0.6s';

    cdContainer.appendChild(newCdImage);

    requestAnimationFrame(() => {
        newCdImage.style.opacity = '1';
        newCdImage.style.transform = 'translateX(0) scale(1)';
        cdImage.style.opacity = '0';
    });

    setTimeout(() => {
        cdImage.src = newCdImage.src;
        cdImage.style.opacity = '1';
        cdImage.style.transform = 'scale(1)';
        cdContainer.removeChild(newCdImage);

        // Fade in genre title smoothly
        genreTitle.style.transition = 'opacity 0.6s ease-in';
        genreTitle.style.opacity = '1';

        // Load new playlist without breaking animation flow
